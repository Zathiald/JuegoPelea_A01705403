...
//poder hacer referencia a esta clase
#include "bowser.h" //Importo el archivo de bowser para
//poder hacer referencia a esta clase
#include "ganondorf.h" //Importo el archivo de ganondorf para
//poder hacer referencia a esta clase
#include "sephiroth.h" //Importo el archivo de sephiroth para
//poder hacer referencia a esta clase
...
int main() {
    ...
    while (jugador[0]->getvidaJug() > 0 && contrin[0]->getvidaCont() > 0 && jugador[0]->getnivelJug() == 1) {
        ...
        //Se crea un if para cuando el jugador escoge el primer ataque
        if (opcionAtaque == 1) { 
            //Se crea un if para cuando el enemigo usa su primer ataque
            if (ataqueEnemigo == 0) {
                //Se crea un if para cuando el ataque del enemigo falla
                if (contrincanteChance == 0) {
                    //Se crea un if para cuando el ataque del jugador falla
                    if (jugadorChance == 2) {
                        cout << "TU ATAQUE HA FALLADO...pero Bowser tampoco logro hacer nada" << endl;
                    }
                    //Se crea un if para cuando el ataque del jugador funciona
                ...
                //Se crea un if para cuando el ataque del enemigo funciona
                    ...
                    //Se crea un if para cuando el ataque del jugador tambien funciona, creando un contraataque
                    else if (jugadorChance == 0 || jugadorChance == 1) {
                ...
                //Se crea un if para cuando el enemigo se defiente
            ...
            //Se crea un if para cuando el enemigo usa su segundo ataque
        ...
        //Se crea un if para cuando el jugador decide usar su segundo ataque
        ...
        //Este if analiza si el enemigo llego a vida 0 o menor a 0, si se logro se sube de nivel al jugador
        ...
        // Este if analiza si el jugador llego a vida 0 o menor a 0, si esto pasa se termina el juego
        else if (jugador[0]->getvidaJug() <= 0) {
            cout << nombreJug << " ESTA FUEERAAAAA, SE ACABO" << endl;
            return 0;
        }
        //Si tanto el jugador y el enemigo aun tienen vida, se sigue a el turno del enemigo a atacar
        else {

            //Se muestra un mensaje al usuario sobre si quiere atacar o defenderse
            ...
            if (decision == "D") {
                int ataqueJugador;
                ataqueJugador = rand() % 1; //Es un número random 
                //entre 0 y 1, solo para llamar a la función de atacarJugador
                //este número al final no hara nada

                contrin[0]->atacarJugador("D", ataqueJugador, jugador[0], contrin[0]); //Se llama a la función de atacarJugador con la opción de haber defendido
            }

            //Se crea un if para si el jugador decidio atacar
            else if (decision == "A") {

                int ataqueJugador; //Es ún numero que nos determinara que número de ataca usa el jugador 
                ...
                //Si el jugador decide 1, se llama la funcion de atacarJugador con la opcion de Ataque y de primer ataque
                if (ataqueJugador == 1) {
                    contrin[0]->atacarJugador("A", 1, jugador[0], contrin[0]);
                }
                //Si el jugador decide 1, se llama la funcion de atacarJugador con la opcion de Ataque y de segundo ataque
...
//----------------------------------------------------------------------------------------------------------------------------------
    ...
    while (jugador[0]->getvidaJug() > 0 && contrin[1]->getvidaCont() > 0 && jugador[0]->getnivelJug() == 2) {
        int contrincanteChance, jugadorChance;
        float vidaCont, vidaJug;
        string decision;
        int ataqueEnemigo, opcionAtaque;
        ...
        if (contrin[1]->getvidaCont() <= 0) {
            ...
            cout << "D-Defender" << endl;
            cin >> decision;
            while (decision != "A" && decision != "D" && decision != "a" && decision != "d") {
                cout << "Escoge una opcion correcta" << endl;
                cin >> decision;
            }
            if (decision == "D") {
    ...
    while (jugador[0]->getvidaJug() > 0 && contrin[2]->getvidaCont() > 0 && jugador[0]->getnivelJug() == 3) {
        int contrincanteChance, jugadorChance;
        float vidaCont, vidaJug;
        string decision;
        int ataqueEnemigo, opcionAtaque;
